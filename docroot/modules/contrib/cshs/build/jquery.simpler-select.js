  ...
  var ClientSideHierarchicalSelect = /*#__PURE__*/function () {
    ...
    _createClass(ClientSideHierarchicalSelect, [{
      ...
      value: function createSelectElement($element, parent) {
        ...
        var describedByValue = $element.attr('aria-describedby');
        var inMutiValueFormFirstRow = $element.closest("tbody tr:first-of-type").length > 0;
        var requiredValue = $element.attr('required');
        ...
        var $select = $createElement('select').addClass(this.elementClasses).attr({
          'id': selectId,
          'aria-describedby': describedByValue !== 'undefined' ? describedByValue : null,
          'required': inMutiValueFormFirstRow !== false && requiredValue === 'required' ? "required" : null,
          'aria-required': inMutiValueFormFirstRow !== false && requiredValue === 'required' ? "true" : null
        ...
        if (this.settings.labels[this.currentLevel]) {
          ...
          var defaultLabel = $('label[for="' + this.elementId + '"]').text() + " Level " + this.currentLevel;
          $createElement('label').addClass('visually-hidden').attr('for', selectId).text(defaultLabel).appendTo($wrapper);
        }
