...
(($, wrapperClass, pluginName): void => {
  ...
  class ClientSideHierarchicalSelect implements Cshs.Plugin {
    ...
    protected createSelectElement<T extends JQuery>($element: T, parent?: string, addNoneOption = true): T | JQuery<HTMLDivElement> {
      ...
      const describedByValue = $element.attr('aria-describedby');
      const inMutiValueFormFirstRow = $element.closest("tbody tr:first-of-type").length > 0;
      const requiredValue = $element.attr('required');
      ...
      if (this.settings.labels[this.currentLevel]) {
        ...
        const defaultLabel = $('label[for="' + this.elementId + '"]').text() + " Level " + this.currentLevel;
        $createElement('label')
