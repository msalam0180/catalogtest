...
namespace Drupal\content_moderation_state_relationship;
...
class ViewsData {
  ...
  public function alterViewsData(array &$data) {
    ...
    foreach ($entity_types_with_moderation as $entity_type_id => $entity_type) {
      ...
      $data[$table]['moderation_state']['relationship'] = [
        'id' => 'standard',
      ...
      $data[$revision_table]['moderation_state']['relationship'] = [
        'id' => 'standard',
        'label' => $this->t('@label moderation state', ['@label' => $entity_type->getLabel()]),
