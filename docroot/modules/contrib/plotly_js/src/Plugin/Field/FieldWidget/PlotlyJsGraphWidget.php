...
class PlotlyJsGraphWidget extends WidgetBase implements ContainerFactoryPluginInterface {
  ...
  private function loadExternalPlotlyTemplateFile($plotname) {
    ...
    try {
      ...
      $this->messenger()->addError($this->t('Error: Unable to load graph template for %template: invalid Yaml. %exception', $message_data), 'error');
      return [];
    }
  }
  ...
  private function processFormTemplateFields(array $data, array $element_parents, array $visibility_references, $delta, FormStateInterface $form_state) {
    ...
    foreach ($data as $field_data) {
      ...
      if (!isset($field_data['fieldname'])) {
        ...
        $this->messenger()->addError($this->t('Error: Plotly found a broken field definition: @fieldname.', $message_data), 'error');
        return [];
      }
          ...
          switch ($field_data['values']) {
            ...
            case 'subplotid':
              $field_definition = [
                '#type' => 'textfield',
                '#size' => 50,
                '#description' => $this->t('Please note that anything other than the default value in this field requires a custom axis implementation in the graph template files.') . ' ',
              ];
              break;
            ...
            default:
              ...
              if ($field_data['fieldname'] != 'type') {
                ...
                $this->messenger()->addError($this->t('Error: Plotly is missing a definition for @fieldname: @values.', $message_data), 'error');
              }
              $field_definition = [];
              break;
          }
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    if (!$configuration_settings['use_external'] && !file_exists(DRUPAL_ROOT . '/' . $install_path)) {
      ...
      $this->messenger()->addError($this->t('Error: Plotly.Js has not been installed. Please download from <a href=":remotepath">:remotepath</a> and install at %installpath, or visit the <a href=":config">Plotly.Js settings page</a> and choose an external file.', $message_data), 'error');
    }
